...
class OrderViews(APITestCase):
    ...
    def test_get_order_sucess_200(self):
        data_transaction ={
        ...
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.token_admin)
        response = self.client.get('/api/order/', data_transaction)
        output = response.json()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(output), 1)

    def test_get_one_order_por_transaction_sucess_200(self):
        data_transaction ={
            "payment": {"id":self.id_payment },
	        "products": [{"id":self.id_products , "quantity": 10}]
        }
        ...
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.token_admin)
        response = self.client.get(f'/api/order/transaction/{id}/', data_transaction)
        output = response.json()
        self.assertEqual(response.status_code, 200)
